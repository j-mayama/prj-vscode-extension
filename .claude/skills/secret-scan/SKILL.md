...
FILES=$(git ls-files -co --exclude-standard \
  ...
  | grep -vE '\.(png|jpg|jpeg|gif|ico|woff2?|ttf|vsix|wav|mp3)$')
...
npx --yes \
  ...
  secretlint "**/*"                 # 秘密なし=exit 0 / 検出=exit 1
```
...
echo "$FILES" | xargs grep -niE 'C:\\\\Users\\\\[^\\\\]+|/home/[^/]+|/Users/[^/]+' 2>/dev/null
...
| バージョン | 日付       | 変更内容                                                                                         |
| ---------- | ---------- | ------------------------------------------------------------------------------------------------ |
| 1.0.0      | 2026-07-14 | frontmatter と変更履歴を追加。secretlint のルール取得を明示し、Linuxホームパスの正規表現を修正。テキスト形式の SVG をスキャン対象に変更 |
