...
def _ensure_shell_lf_gitattributes(project_root: Path) -> str:
...
def install(
    *,
    ...
    if template_type == "wp":
        src = _resolve_template_dir(project_root, "wp")
        ...
        src = _resolve_template_dir(project_root, "node")
    ...
    for path in src.rglob("*"):
        ...
        if _is_text_file(path):
            ...
            if out_path.suffix.lower() == ".sh":
                ...
                out_path.write_text(transformed, encoding="utf-8", newline="\n")
            else:
                out_path.write_text(transformed, encoding="utf-8", newline="")
        else:
    ...
    gitattributes_path = _ensure_shell_lf_gitattributes(project_root)
