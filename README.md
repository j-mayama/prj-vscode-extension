...
| スキル | できること |
| --- | --- |
| **`/codex-review`** | OpenAI Codex CLI（`codex exec review`）による独立コードレビュー＆修正ループ。指摘ごとに「対応 / 見送り」をダイアログで選択し、採用した指摘だけを Claude が修正して、指摘ゼロになるまで再レビューを繰り返す。使用モデルとエフォートは初回に対話で設定して永続化する。Stop hook を使えば、応答のたびに未レビューの差分を検知して自動でループを回すこともできる（要: Codex CLI と ChatGPT 認証） |
